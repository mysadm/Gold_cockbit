...
**Cold-cache search share of p50, one line.** Standard: 157ms of 9689ms ≈ **1.6%**; personalized: 109ms of 15451ms ≈ **0.7%** — an order of magnitude above the prior (warm-cache) round's ~0%, but evidence search is still a small minority of latency even in the guaranteed-cold, first-request/just-restarted case; model generation time remains the dominant cost by a wide margin.

**Phase 2 retry investigation — approved fixes implemented, 2026-09-26.** Phases 3-5 stay paused. All four approved items:

1. **Deterministic formatting repair**, `server/providers/formatRepair.mjs` (new), wired into `validatedAnalysis.mjs` between a failed validation and the retry loop: over-length strings cut at the last sentence/word boundary (never mid-word — verified for Arabic too, space-delimited like English), excess array items capped to the first N in model order, unknown fields stripped, empty/whitespace `data_flags` entries dropped. `completion was truncated` is excluded from repair (stays a retry — the JSON itself may be incomplete). Re-validated after repair; only the errors that remain (if any) drive a real retry. Every attempt's `repaired`/`repairedFields` is logged in `[analyst-v4-metrics]` regardless of outcome. `shared/analystOutputV4.mjs` gained an exported `FIELDS` map (single source of truth for allowed keys, used by both the validator's unknown-field checks and the repairer, so they can't drift apart). Tests: `tests/server/format-repair.test.mjs` (new, 18 cases: every repair type, an Arabic word-boundary case, the no-safe-boundary edge case, semantic fields left untouched) + `validated-analysis.test.mjs` (repair avoids a second provider call entirely; repair-plus-remaining-semantic-error still retries, with the correction prompt only mentioning what's left; truncation is never repaired).

2. **Semantic errors** (weight math, unknown EV-ID, invalid enum, the new DCA-limit check below) still always retry — no prompt changes made; Appendix A's PROMPT_V2 is otherwise untouched pending real semantic-error data (none observed yet — see below).

3. **DCA safety regression restored — chose text-scanning, not a structured field.** V4 had dropped v3's check that a `reads.dca`/`dca_read` amount never exceeds `current_installment_limit_egp`. Considered adding an optional numeric `dca_read.amount_egp` validated against the cap instead: rejected, because it only protects a field nobody reads — a model could still write a bad number in the user-facing `text` while simply omitting `amount_egp`, so the check would guard nothing real. Chose instead: reject any EGP amount stated in `dca_read.text` above the cap, reusing v3's exact, already-proven amount-extraction regex (Arabic-Indic digit normalization, `EGP`/`جنيه` labels) — extracted to a shared `extractEgpAmounts()` in `shared/analystContract.mjs` so v3's `validateV3` and v4's new `validateDcaRead` check share one implementation instead of two copies that could drift. `validateAnalystOutput`/`validateDcaRead` gained an optional `dcaLimitEgp` param (threaded from `runAnalysisV4.mjs`'s `snapshot.dca.current_installment_limit_egp` through `runValidatedAnalysis`). This is semantic per the instruction: never repaired, always a retry on violation — verified by a dedicated test (`validated-analysis.test.mjs`: an over-limit `dca_read.text` retries even though nothing else is wrong). New validator tests in `analyst-output-v4.test.mjs`: over-limit rejected, at-limit accepted, Arabic-Indic digits recognized, and no check at all when no DCA plan exists (`dcaLimitEgp` absent).

4. **Search timeout + stale fallback.** `server/webSearch.mjs`: `SEARCH_TIMEOUT_MS` 8000 → 3000 (real cold search was 100-200ms in every measurement so far — 3s keeps 15-30x headroom while bounding a slow/unreachable SerpAPI far tighter; the old 8s is what produced this repo's one ~8000ms outlier, now explained rather than mysterious). `server/searchCache.mjs` gained `getStale(key)`: an expired entry is no longer deleted on a normal cache-miss lookup, so it stays available as a fallback for up to 24h. On any search failure (timeout or otherwise) `webSearch.mjs` now returns the stale result instead of throwing, when one exists within that window; `evidence.mjs`'s existing `Promise.allSettled` handling is unchanged (a stale-served facet counts as fulfilled, not failed — intentional, since PROMPT_V2's own "if evidence_age_hours is high, flag stale evidence" is the mechanism meant to flag this, not `searchStatus`). `runAnalysisV4.mjs` now sets `evidence_built_at` to the stale entry's original fetch time (not "now") whenever a fallback was used, via a new `staleFallbackAt` field threaded through `collectEvidence`'s shared `searchMetrics` object; `staleFallbackUsed` is logged in `[analyst-v4-metrics]`. Tests added to `tests/server/web-search.test.mjs`: falls back on timeout (a hung fetch that only settles when the abort fires) and on a non-timeout failure, respects the 24h cutoff, records `metrics.staleFallbackAt` correctly; existing tests (including "throws when there is no prior result") unaffected.

**Item 1 retry-inventory gap that motivated all this:** the pre-existing `attempts[]` never recorded a failed attempt's own errors — only the *final* attempt's, empty on any run that ultimately passed — so of the 8 retries observed across every V4 measurement so far, only 1 (the `data_flags` failure that led to the earlier prompt fix) had a recorded reason. `validatedAnalysis.mjs`'s `attempts[].errors` (added, committed separately as logging-only before this implementation) closes that gap for every run from here on.

**Item 5 — measured after, same fixture methodology, real active provider, `npm run test:dev` green first (877/877, only the known `ai-settings-ui` failure).** Standard en/expert ×3, personalized en/expert ×6 (one-off script, not committed):

| tier | run | total ms | search ms | model ms | retries | output tokens | repaired? |
|---|---|---|---|---|---|---|---|
| standard | 1 | 9722 | 2141 | 7580 | 0 | 610 | yes — `data_flags (item truncated)` |
| standard | 2 | 7268 | 41 | 7227 | 0 | 567 | no |
| standard | 3 | 6586 | 34 | 6552 | 0 | 517 | no |
| personalized | 1 | 8834 | 33 | 8800 | 0 | 760 | yes — `text (truncated)` (dca_read) |
| personalized | 2 | 8111 | 134 | 7977 | 0 | 672 | no |
| personalized | 3 | 8061 | 30 | 8031 | 0 | 704 | no |
| personalized | 4 | 9531 | 43 | 9488 | 0 | 725 | no |
| personalized | 5 | 9228 | 34 | 9194 | 0 | 786 | no |
| personalized | 6 | 9698 | 28 | 9669 | 0 | 831 | yes — `data_flags (item truncated)` |

**Retry rate: 0/9 (0%).** Every first-attempt error observed: none reached the "still an error after repair" state — the only errors that occurred (3 of 9 attempts, all `data_flags`/`dca_read.text` length) were fixed by the formatting repair before validation was even re-checked against what would count as a retry trigger, so no run needed a second model call at all this round.

| | n | p50 (now) | p95 (now) | p50 (previous V4 rounds, same en/expert combo) | p95 (previous) | token p50 (now) |
|---|---|---|---|---|---|---|
| standard | 3 | 7268ms | 9722ms | 15821ms (n=6, rounds 1+2) | 24941ms | 567 |
| personalized | 6 | 8834ms | 9698ms | 15451ms (n=9, all 3 prior rounds) | 20395ms | 725 |
