...
**Phase 1 fix (4/5) — DCA verdict shape: not changed, awaiting approval.** Checked `shared/analystContract.mjs` and the UI (`src/`, `flutter_app/` — flutter has no DCA-output usage at all): the *only* existing personalized-DCA output surface is `reads.dca`, an optional free-text string (`shared/analystContract.mjs:10,140,142-146`) rendered client-side as a `ClaimField {text, evidence_ids}` (`src/lib/analyst.ts:36,69,253-254,350,362,453`; `src/App.tsx:1927-1930`). There is no status enum anywhere today. Proposed smallest V4 addition, matching that shape 1:1 (renaming `evidence_ids`→`ev_ids` for consistency with the rest of V4): `"dca_read": {"text": string ≤200 chars, "ev_ids": string[] 0–3, must exist in the pack}`. This was presented to the user for approval and not yet applied — `schemas/analyst_output.personalized.schema.json`'s current `dca: {status, note}` is explicitly marked as a placeholder pending that decision (see its `$comment`); `shared/analystOutputV4.mjs`'s `validateDca` is unchanged pending the same.
