...
**Phase 2 — measured before/after, ANALYST_V4=1, dev API, real active provider (`claude`/`claude-haiku-4-5-20251001`), 2026-09-26.** One-off script (not committed, same methodology as Phase 1's max_tokens measurement): direct `runAnalysisV3` calls (routed to v4) against the checked-in fixture, per tier — "standard" = fixture reshaped to the real market-tier shape (`analysis_scope:'market'`, wallet empty, `dca:null`; note real standard-tier snapshots are always `explanation_level:'beginner'` per `buildMarketSnapshot`, so the "en/expert" standard rows below are a synthetic combination for measurement only, not a real reachable state) — "personalized" = fixture as-is. `searchMs` differs sharply between tiers only because `evidence.mjs`'s web-search queries are cached in-process by query text and the standard-tier runs went first, warming the cache for the personalized runs that followed a few seconds later — not a v4 behavior change.

| tier | locale/level | total ms | retries | output tokens per attempt | validation |
|---|---|---|---|---|---|
| standard | en/expert | 24941 | 1 | 625, 703 | ok |
| standard | en/expert | 22495 | 1 | 595, 570 | ok |
| standard | en/expert | 15821 | 0 | 601 | ok |
| standard | ar/beginner | 16219 | 0 | 766 | ok |
| standard | ar/beginner | 14996 | 0 | 930 | ok |
| personalized | en/expert | 10050 | 0 | 798 | ok |
| personalized | en/expert | 20395 | 1 | 785, 834 | **failed** (`data_flags must be an array of up to 5 short strings`, on both attempts) |
| personalized | en/expert | 17437 | 1 | 726, 723 | ok |
| personalized | ar/beginner | 12115 | 0 | 1060 | ok |
| personalized | ar/beginner | 10426 | 0 | 1057 | ok |

| | n | p50 | p95 | pass rate | output tokens/attempt (min–max) |
|---|---|---|---|---|---|
| **standard, v4 (this run)** | 5 | 16219ms | 24941ms | 5/5 | 570–930 |
| standard, v3 baseline (Phase 0, real scheduled runs) | 13 | ≈17.8s | ≈27.8s | n/a (no per-run validation logged pre-Phase-1) | n/a (not persisted pre-Phase-3) |
| **personalized, v4 (this run)** | 5 | 12115ms | 20395ms | 4/5 | 723–1060 |
| personalized, v3 baseline (Phase 0) | 5 | ≈12.45s | ≈28.44s | 5/5 | 743–2144 |

Reading these: output tokens per attempt are markedly smaller than v3's baseline range (personalized: 723–1060 vs. 743–2144), as intended by shrinking the schema; p50 latency is roughly flat to modestly better (model think/generate time dominates, not token count alone), while p95 improved more visibly for the personalized tier (less tail risk from the old free-form `reads`/`assumptions` fields). Sample sizes (5 per cell) are the human's specified minimum for this phase, not enough to call any of this statistically significant — Phase 7's ≥20-per-tier comparison is the real before/after. **Finding for Phase 7:** one personalized run failed validation on `data_flags` twice in a row (the correction retry did not fix it) — worth checking whether PROMPT_V2's `data_flags[]` guidance needs an explicit example or count reminder once Phase 7 has a larger sample to judge a real pass-rate impact.

**Phase 2 — tests.** `npm run test:dev`: 89/90 files, 846/846 tests pass (only the known pre-existing `ai-settings-ui` failure remains, per Step 0). New: `tests/server/run-analysis-v4.test.mjs` (both tiers end-to-end on the old evidence source, PROMPT_V2/jsonSchema/maxTokens wiring, DCA trimming, precomputes, EV-ID pattern remap, per-attempt usage/retries, truncation, admin-prompt-override-is-ignored). Extended: `tests/server/analyst-output-v4.test.mjs` (`computePriorStateEligible`, 4 cases), `tests/server/validated-analysis.test.mjs` (`attempts`/truncation/`onRawAnswer`), `tests/server/compact-provider.test.mjs` (v4 budget, no 4096 floor), `tests/server/claude-provider.test.mjs` (cache_control only under `compact:'v4'`), `tests/server/dispatch.test.mjs` (`maxTokens` override). Updated to stub `ANALYST_V4` off (mirroring Step 0's `DISABLE_NOTIFICATIONS`/`DISABLE_SCHEDULER` fix, since `.env.dev` now sets it): `run-analysis-v3.test.mjs`, `analyze-route.test.mjs`, `correction-hints.test.mjs`, `analysis-routes.test.mjs` — each exercises the real v3 pipeline and would otherwise be silently rerouted to v4 under `npm run test:dev`.
