...
function primaryActionColor(action: PrimaryDecisionAction): string {
  switch (action) {
    case 'buy':
      return 'var(--up)';
    case 'reduce':
...
function confidenceColor(confidence: AIConfidenceLevel): string {
  switch (confidence) {
    case 'high':
      return 'var(--up)';
    case 'medium':
...
function loadState(keys: StorageKeys): AppState {
  ...
  try {
    ...
    // .reasons / etc., so restoring a v1 blob here crashes the AI tab on
    // load for any returning user. schema_version: '2' is a reliable
    // discriminator (v1 objects never had it) — anything that fails it is
    // treated as if no analysis had ever been run.
    const savedAi = saved?.ai;
    const restoredData =
      savedAi?.data && savedAi.data.schema_version === '2' && savedAi.data.primary_decision
        ? savedAi.data
        : null;
    return {
      ...defaultState,
      ...
      ai: { ...defaultState.ai, ...savedAi, data: restoredData, loading: false, startedAt: null },
...
function App({ user, onLogout }: { user: CurrentUser; onLogout: () => void }) {
  ...
  const refreshProviders = () => {
    const load = isAdmin ? listProviders() : fetchActiveProvider().then((p) => (p ? [p] : []));
    load.then(setProviders).catch(() => {});
  };
  ...
  const providerTypeLabel = (type: ProviderType) =>
    ({ ollama: t.settingsTypeOllama, openai: t.settingsTypeOpenAI, claude: t.settingsTypeClaude, openrouter: t.settingsTypeOpenRouter, custom: t.settingsTypeCustom, shared: t.settingsTypeShared }[type]);
  ...
  // 24k-gram-equivalent of all wallet holdings, derived from the wallet tab's own conversion
  // (walletIntlValue already sums amount × per-unit price, each priced off g24 via the karat
  // fractions in walletRows — dividing back out by g24 recovers grams-at-24k without
  // reimplementing the oz/g21/g18/pound karat math here).
  const walletGramsAt24k = g24 > 0 ? walletIntlValue / g24 : 0;

  const sensitivityTable = useMemo(
    () => computeSensitivityTable({ spot: state.spot, egp: state.egp, gramsAt24k: walletGramsAt24k }),
    [state.spot, state.egp, walletGramsAt24k]
  );
  const sensitivityMoves = useMemo(() => Array.from(new Set(sensitivityTable.map((r) => r.usd_egp_move_pct))), [sensitivityTable]);
  const sensitivityXauMoves = useMemo(() => Array.from(new Set(sensitivityTable.map((r) => r.xau_usd_move_pct))), [sensitivityTable]);
  ...
  const analyze = async () => {
    if (!activeProvider) {
      setState((prev) => ({
        ...prev,
        ai: { ...prev.ai, loading: false, error: isAdmin
    ...
    try {
      ...
      // reasons) would falsely claim the whole analysis is a fallback. Use
      // this stripped-down object wherever normalizeAIResult is asked to
      // fill in gaps in a real response; the catch block below still uses
      // `fallback` directly, where the honest self-describing wording
      // belongs (that path genuinely is a fallback with no live model data).
      const normalizationFallback: AIResultV2 = { ...fallback, primary_decision: { ...fallback.primary_decision, reasons: [] }, dca_read: undefined };
          ...
          : normalizeAIResult(
              {
                primary_decision: {
                  headline: cleanText
                    ? (state.lang === 'ar' ? `ملخص من الرد: ${cleanText.slice(0, 180)}` : `Summary from the model reply: ${cleanText.slice(0, 180)}`)
                    : undefined,
                  reasons: cleanText ? [{ text: cleanText.slice(0, 320), evidence_ids: [] }] : undefined,
                },
                suggested_weights: fallback.suggested_weights,
                weights_reasoning: {
                  text: state.lang === 'ar' ? 'تمت صياغة هذا التقرير من النص المجاني الذي أعاده المحلل.' : 'This report was derived from the free-text reply returned by the analyst.',
                  evidence_ids: [],
                },
  ...
  // evidence_ids as small inline "[EV-001]" chips.
  const renderClaimField = (field: ClaimField, fontSize = 15) => (
    <div className="soft-text" style={{ fontSize, lineHeight: 1.8 }}>
      {field.text}
      {state.aiLevel === 'expert' && field.evidence_ids.length ? (
        <span className="muted-text font-mono" style={{ fontSize: 12, marginInlineStart: 6 }}>
          {field.evidence_ids.map((id) => `[${id}]`).join(' ')}
        </span>
      ) : null}
    </div>
  );
                  ...
                  <div style={{ marginTop: 16, borderTop: '1px solid var(--border)', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 6 }}>
                        <div className="section-label gold-text">{t.aiPrimaryH}</div>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <span
                            ...
                            style={{
                              ...
                              color: primaryActionColor(state.ai.data.primary_decision.action),
                            }}
                          >
                            {t.aiActionVal[state.ai.data.primary_decision.action]}
                          </span>
                          <span className="font-mono muted-text" style={{ fontSize: 11, padding: '2px 8px', borderRadius: 999, border: '1px solid var(--border)' }}>
                            {t.aiHorizonVal[state.ai.data.primary_decision.horizon]}
                          </span>
                          <span
                            className="font-mono"
                            style={{
                              ...
                              borderRadius: 999,
                              border: '1px solid var(--border)',
                              color: confidenceColor(state.ai.data.primary_decision.confidence),
                            }}
                          >
                            {t.aiConfidenceLbl}: {t.aiConfidenceVal[state.ai.data.primary_decision.confidence]}
                          </span>
                        </div>
                      </div>
                    ...
                    {state.ai.data.primary_decision.reasons.length ? (
                      <div>
                        ...
                        {state.ai.data.primary_decision.reasons.map((reason, i) => (
                          <div key={i} className="soft-text" style={{ fontSize: 15, lineHeight: 1.8 }}>
                            • {reason.text}
                            {state.aiLevel === 'expert' && reason.evidence_ids.length ? (
                              <span className="muted-text font-mono" style={{ fontSize: 12, marginInlineStart: 6 }}>
                                {reason.evidence_ids.map((id) => `[${id}]`).join(' ')}
                              </span>
                            ) : null}
                          </div>
                        ))}
                    ...
                    {state.aiLevel === 'expert' && state.ai.data.horizon_actions.length ? (
                      <div>
                        <div className="section-label gold-text" style={{ marginBottom: 6 }}>{t.aiHorizonActionsH}</div>
                        {state.ai.data.horizon_actions.map((ha, i) => (
                          <div key={i} style={{ padding: '6px 0', borderBottom: i < state.ai.data!.horizon_actions.length - 1 ? '1px dashed var(--border)' : 'none' }}>
                            <div style={{ display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' }}>
                              <span className="font-mono gold-text" style={{ fontSize: 12 }}>{t.aiHorizonVal[ha.horizon]}</span>
                              <span className="soft-text" style={{ fontSize: 14, fontWeight: 600 }}>{ha.action}</span>
                            </div>
                            {ha.condition ? (
                              <div className="muted-text" style={{ fontSize: 13, marginTop: 2 }}>{t.aiConditionLbl}: {ha.condition}</div>
                            ) : null}
                          </div>
                        ))}
                      </div>
                    ...
                    {state.ai.data.suggested_weights ? (
                      <div>
                        ...
                        {(() => {
                          const block = applyBlockReason(state.ai.validation?.ok, state.ai.data?.primary_decision?.action);
                          return block ? (
                            <div className="down-text" style={{ fontSize: 12, marginTop: 4 }}>{block === 'no_evidence' ? t.aiApplyNoEvidence : t.aiApplyDisabled}</div>
                          ) : null;
                        })()}
                      </div>
                    ) : null}
                    {state.ai.data.egp_read ? (
                      <div>
                        ...
                        {renderClaimField(state.ai.data.egp_read)}
                      </div>
                    ) : null}
                    {state.ai.data.wallet_read ? (
                      <div>
                        <div className="section-label gold-text" style={{ marginBottom: 6 }}>{t.aiWalletH}</div>
                        {renderClaimField(state.ai.data.wallet_read)}
                      </div>
                    ) : null}
                    {state.ai.data.dca_read ? (
                      <div>
                        <div className="section-label gold-text" style={{ marginBottom: 6 }}>{t.aiDcaH}</div>
                        {renderClaimField(state.ai.data.dca_read)}
                      </div>
                    ) : null}
                    {state.ai.data.watchlist_read ? (
                      <div>
                        <div className="section-label gold-text" style={{ marginBottom: 6 }}>{t.aiWatchH}</div>
                        {renderClaimField(state.ai.data.watchlist_read)}
                      </div>
                    ) : null}
                    {state.aiLevel === 'expert' && state.ai.data.assumptions.length ? (
                      <div>
                        <div className="section-label gold-text" style={{ marginBottom: 6 }}>{t.aiAssumptionsH}</div>
                        {state.ai.data.assumptions.map((item) => <div key={item} className="soft-text" style={{ fontSize: 14, lineHeight: 1.8 }}>• {item}</div>)}
                      </div>
                    ) : null}
                    {state.aiLevel === 'expert' && state.ai.data.missing_inputs.length ? (
                      <div>
                        <div className="section-label gold-text" style={{ marginBottom: 6 }}>{t.aiMissingH}</div>
                        {state.ai.data.missing_inputs.map((item) => <div key={item} className="soft-text" style={{ fontSize: 14, lineHeight: 1.8 }}>• {item}</div>)}
                      </div>
                    ) : null}
                    {state.aiLevel === 'expert' && sensitivityTable.length ? (
                      <div>
                        <div className="section-label gold-text" style={{ marginBottom: 6 }}>{t.aiSensitivityH}</div>
                        <div className="muted-text" style={{ fontSize: 12, marginBottom: 8, lineHeight: 1.6 }}>{t.aiSensitivityNote}</div>
                        <div style={{ overflowX: 'auto' }}>
                          <div style={{ display: 'flex', fontSize: 12, fontWeight: 600, letterSpacing: '0.04em', color: 'var(--text-muted)', padding: '0 0 8px' }}>
                            <span style={{ flex: '0 0 64px' }} />
                            {sensitivityMoves.map((egpMove) => (
                              <span key={egpMove} className="font-mono" style={{ flex: '0 0 78px', textAlign: 'end' }}>
                                {egpMove > 0 ? '+' : ''}{egpMove}%
                              </span>
                            ))}
                          </div>
                          {sensitivityXauMoves.map((xauMove) => (
                            <div key={xauMove} style={{ display: 'flex', alignItems: 'center', padding: '6px 0', borderTop: '1px solid var(--border)' }}>
                              <span className="font-mono gold-text" style={{ flex: '0 0 64px', fontSize: 12 }}>
                                {xauMove > 0 ? '+' : ''}{xauMove}%
                              </span>
                              {sensitivityMoves.map((egpMove) => {
                                const cell = sensitivityTable.find((r) => r.xau_usd_move_pct === xauMove && r.usd_egp_move_pct === egpMove);
                                const isBase = xauMove === 0 && egpMove === 0;
                                return (
                                  <span
                                    key={egpMove}
                                    className="font-mono"
                                    style={{ flex: '0 0 78px', textAlign: 'end', fontSize: 13, color: isBase ? 'var(--gold)' : 'var(--text)', fontWeight: isBase ? 700 : 400 }}
                                  >
                                    {cell ? fmt(cell.wallet_value_egp) : '-'}
                                  </span>
                                );
                              })}
                            </div>
                    ...
                    {state.ai.validation && !state.ai.validation.ok ? (
                      <div className="down-text" style={{ fontSize: 13, lineHeight: 1.6 }}>
                        ...
                        {state.ai.validation.errors.join(' · ')}
                      </div>
                    ) : null}
...
const T = {
  ...
  en: {
    ...
    aiUsingProvider: 'Using provider', aiNoProvider: 'No active provider — go to Settings', aiNoProviderUser: 'No AI provider is set up yet — ask the admin', aiQuotaLabel: 'Your analyses left today', settingsHeading: 'AI Model Settings', settingsAddHeading: 'Add / Edit Provider', settingsEmpty: 'No providers configured yet.', settingsTypeLabel: 'Type', settingsLabelLabel: 'Label', settingsBaseUrlLabel: 'Base URL', settingsApiKeyLabel: 'API key', settingsApiKeyUnchangedPh: 'Leave blank to keep unchanged', settingsModelLabel: 'Model', settingsSaveBtn: 'Save', settingsCancelBtn: 'Cancel', settingsActivateBtn: 'Set active', settingsActiveBadge: 'Active', settingsEditBtn: 'Edit', settingsDeleteBtn: 'Delete', settingsTypeOllama: 'Ollama (local)', settingsTypeShared: 'Shared (Claude Haiku, daily quota)', settingsSharedNote: 'Server-managed tier — model and connection are pre-configured, no setup needed.', settingsTypeOpenAI: 'OpenAI', settingsTypeClaude: 'Claude', settingsTypeOpenRouter: 'OpenRouter', settingsTypeCustom: 'Custom', settingsTestBtn: 'Test connection', settingsTesting: 'Testing…', settingsTestSuccess: '✓ Connection worked —', settingsTestError: '✗ Connection failed —', settingsValidationError: 'Label and Model are required',
