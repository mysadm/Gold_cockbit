...
export async function runProviderAnalysis(providerRow, prompt, { expectJson = true } = {}) {
  ...
  if (providerRow.provider_type === 'claude') {
    return callClaude({ apiKey: providerRow.api_key, model: providerRow.model, prompt, temperature, maxTokens, expectJson });
  }
  ...
  if (providerRow.provider_type === 'shared') {
    ...
    return callClaude({ apiKey, model: SHARED_TIER_MODEL, prompt, allowWebSearch: false, expectJson });
  }
