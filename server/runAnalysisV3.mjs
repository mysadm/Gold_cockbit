...
export async function runAnalysisV3(provider, input, runProvider, {signal}={}) {
  ...
  let parsed,validation,retries=0,usage=null;
  ...
  const metrics={contract:'3',providerType:provider.provider_type,searchMs,totalMs:Date.now()-started,retries,usage,validationOk:validation.ok,inputCharacters:GOLD_MARKET_ANALYST_SYSTEM_PROMPT.length+prompt.length,outputCharacters:JSON.stringify(parsed).length};
